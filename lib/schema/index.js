        ...
        IF NOT is_deleted THEN
          ...
          LOOP
             IF data ? _key AND data ->> _key IS NOT NULL THEN
              ...
              data := jsonb_set(data, CONCAT('{', _key, '}')::text[], to_jsonb(pgp_sym_encrypt((data ->> _key)::text, encryption_key::text, 'compress-algo=1, cipher-algo=aes256')::text), true);
             /*
        ...
        IF NOT is_deleted THEN
          ...
          LOOP
             IF data ? _key AND data ->> _key IS NOT NULL THEN
               ...
               data := jsonb_set(data, CONCAT('{', _key, '}')::text[], to_jsonb(pgp_sym_decrypt((data ->> _key)::text::bytea, encryption_key::text, 'compress-algo=1, cipher-algo=aes256')::text), true);
              /*
