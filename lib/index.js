...
class PostgresPersistenceEngine extends AbstractPersistenceEngine {
  constructor (connectionStringOrConnection, {
    ...
    super();
    this.connectionStringOrConnection = connectionStringOrConnection;
    ...
    this.db = (async () => {
      ...
      if (createIfNotExists) {
        await db.none(create(tablePrefix, schema, eventTable, snapshotTable, persistenceKey, sequenceNr)).catch(console.error);
      }
      return db;
    })();
  }
  ...
  static mapDbModelToDomainModel (dbEvent, {
    ...
    return new PersistedEvent(
      dbEvent[data],
      Number.parseInt(dbEvent[sequenceNr]),
      dbEvent[persistenceKey],
      dbEvent[tags],
      Number.parseInt(dbEvent[createdAt]),
      !!dbEvent[isDeleted],
      dbEvent[annotations],
      dbEvent[metadata]
    );
  ...
  static mapDbModelToDomainModelEncryption (dbEncryption, {
    ...
    return new PersistedEventEncryption(
      ...
      Number.parseInt(dbEncryption[createdAt]),
      Number.parseInt(dbEncryption[deletedAt]),
      !!dbEncryption[isDeleted]
    );
  }
  ...
  static mapDbModelToSnapshotDomainModel (dbSnapshot, {
    ...
    if (dbSnapshot) {
      return new PersistedSnapshot(
        dbSnapshot[data],
        Number.parseInt(dbSnapshot[sequenceNr]),
        dbSnapshot[persistenceKey],
        Number.parseInt(dbSnapshot[createdAt])
      );
    }
  }
  ...
  init () {
    this.db = (async () => {
      let db = this.connectionStringOrConnection;
      if (typeof (this.connectionStringOrConnection) === 'string') {
        db = pgp()(this.connectionStringOrConnection);
      }
      if (this.createIfNotExists) {
        await db.none(create(this.tablePrefix, this.schema, this.eventTable, this.snapshotTable, this.persistenceKey, this.sequenceNr)).catch(console.error);
      }
      return db;
    })();
  }
  ...
  events (persistenceKey, offset = 0, limit = null, tags) {
    ...
    const query = `
      SELECT * FROM ${this.schema ? this.schema + '.' : ''}${this.tablePrefix}${this.eventTable}_view
      WHERE
        ${this.persistenceKey} = $1 AND ${this.sequenceNr} > $2
      ${tags ? 'AND tags @> ($4::text[])' : ''}
      ORDER BY ${this.sequenceNr}
      LIMIT $3
    `;

    const args = [persistenceKey, offset, limit, tags].filter(x => x !== undefined);
    const result = this.db
      .then(db => db.any(query, args))
      .then(results => results.map(r => PostgresPersistenceEngine.mapDbModelToDomainModel(r, {persistenceKey: this.persistenceKey, sequenceNr: this.sequenceNr})));
  ...
  async latestEncryption (persistenceKey) {
    ...
    return (await this.db).oneOrNone(query, [persistenceKey])
      .then(v => PostgresPersistenceEngine.mapDbModelToDomainModelEncryption(v, {persistenceKey: this.persistenceKey, sequenceNr: this.sequenceNr}));
  }
  ...
  async latestSnapshot (persistenceKey) {
    ...
    const query = `
     SELECT
       *
     FROM ${this.schema ? this.schema + '.' : ''}${this.tablePrefix}${this.snapshotTable}_view
    WHERE
      ${this.persistenceKey} = $1 AND is_deleted = false
    ORDER BY ${this.sequenceNr} DESC
    LIMIT 1
  `;

    return (await this.db).oneOrNone(query, [persistenceKey]).then(r => PostgresPersistenceEngine.mapDbModelToSnapshotDomainModel(r, {
